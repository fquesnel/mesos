...
namespace allocator {
	...
	private:
		...
		// TODO : merge in single class + optimize
		...
		double computeResourcesWeight(const SlaveID &slaveId, const Resources& resources){
			//TODO: add resources filtering and take into account role/quota
			double result = 0;
			...
			if (cpus.isSome())
				result += resourcesWeights.cpuWeigth * cpus.get();

			if (gpus.isSome())
				result += resourcesWeights.gpuWeigth * gpus.get();

			if (disk.isSome()){
				result += (resourcesWeights.diskWeigth * 
					((double)disk.get().bytes() / (double)Bytes::MEGABYTES));

			}
			if (mem.isSome())
				result += (resourcesWeights.memWeigth * ((double)mem.get().bytes() / (double)Bytes::MEGABYTES));

			return result;
		}

		UnitaryResourceWeight computeUnitaryResourcesProportions(const Resources& resources){
			...
			*/			
			...
			if (disk.isSome()){
				max = std::max(max, (double)disk.get().bytes() / (double)Bytes::MEGABYTES);				
			}
			...
			if (mem.isSome())
				result.memWeigth = max / ((double)mem.get().bytes() / (double)Bytes::MEGABYTES);
			else
				result.memWeigth = 0.0f;

			if (disk.isSome())
				result.diskWeigth = max / ( (double)disk.get().bytes() / (double)Bytes::MEGABYTES);
			else
			...
			"disk=" << result.diskWeigth << " from idle resources " << resources << std::endl;
			return result;
		}
